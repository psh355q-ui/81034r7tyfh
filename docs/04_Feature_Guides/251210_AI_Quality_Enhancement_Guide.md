...
```bash
curl -X POST "http://localhost:8002/ai-quality/signals/record" \
...
```bash
curl -X POST "http://localhost:8002/ai-quality/signals/update-outcomes"
```

#### Get Accuracy

```bash
curl "http://localhost:8002/ai-quality/signals/accuracy?source=claude&lookback_days=30"
```

#### Confidence Calibration

```bash
curl "http://localhost:8002/ai-quality/signals/confidence-calibration?source=claude"
```

#### RAG Impact

```bash
curl "http://localhost:8002/ai-quality/signals/rag-impact?lookback_days=90"
```

#### Source Comparison

```bash
curl "http://localhost:8002/ai-quality/signals/source-comparison?lookback_days=90"
```
...
```bash
curl "http://localhost:8002/ai-quality/ensemble/weights"
```

#### Optimize Weights

```bash
curl -X POST "http://localhost:8002/ai-quality/ensemble/optimize?lookback_days=90&objective=sharpe&method=bayesian"
```

#### Optimization Report

```bash
curl "http://localhost:8002/ai-quality/ensemble/optimization-report"
```
...
```bash
curl "http://localhost:8002/ai-quality/strategy/current"
```

#### Update Strategy

```bash
curl -X POST "http://localhost:8002/ai-quality/strategy/update?force=false"
```

#### Calculate Position Size

```bash
curl "http://localhost:8002/ai-quality/strategy/position-size?ticker=AAPL&signal_confidence=0.85&portfolio_value=100000"
```

#### Validate Signal

```bash
curl -X POST "http://localhost:8002/ai-quality/strategy/validate-signal?signal=BUY&confidence=0.75&sharpe_ratio=1.8"
```

#### Strategy Status

```bash
curl "http://localhost:8002/ai-quality/strategy/status"
```

### 5.4 Dashboard API

```bash
curl "http://localhost:8002/ai-quality/dashboard"
```
