...
### Load Test Results
- **Target**: `http://localhost:8002` (Main Server) & `http://localhost:8002` (Test Server)
- **Observation**:
  - Initial tests against Port 8002 showed successful health checks.
  - Tests against Port 8002 returned `404 Not Found` for new endpoints, indicating the server process has not yet reloaded the latest `main.py` changes.
...
2. Run Locust:
   ```bash
   python -m locust -f backend/tests/load/locustfile.py --host http://localhost:8002
   ```
