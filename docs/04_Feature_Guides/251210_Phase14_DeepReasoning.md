...
docs/
└── 251210_Phase14_DeepReasoning.md       # 이 문서
```
...
```bash
curl -X POST http://localhost:8002/api/v1/reasoning/analyze \
...
```bash
curl http://localhost:8002/api/v1/reasoning/knowledge/Google
```

### GET /api/v1/reasoning/backtest

```bash
curl http://localhost:8002/api/v1/reasoning/backtest
```
...
- [251210_MASTER_GUIDE.md](../251210_MASTER_GUIDE.md) - 전체 프로젝트 가이드
