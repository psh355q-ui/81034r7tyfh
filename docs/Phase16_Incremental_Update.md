...
**수동 업데이트**:
```bash
curl -X POST http://localhost:8000/api/incremental/scheduler/run-now
```

**모니터링**:
```bash
# 전체 통계
curl http://localhost:8000/api/incremental/stats

# 비용 절감 확인
curl http://localhost:8000/api/incremental/cost-savings
```
