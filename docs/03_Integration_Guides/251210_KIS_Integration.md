...
`.env` 파일을 프로젝트 루트에 생성하여 API 키와 설정을 관리합니다. `kis_devlp.yaml`보다 우선순위가 높습니다.
...
KIS_ACCOUNT_NUMBER=12345678-01  # 계좌번호 전체 (또는 앞 8자리)

# KIS Virtual Trading Credentials (Optional)
KIS_PAPER_APP_KEY=your_paper_app_key
KIS_PAPER_APP_SECRET=your_paper_app_secret
KIS_PAPER_ACCOUNT=12345678-01

# System Logic
KIS_ENV=production  # production (Real) or sandbox (Virtual)
KIS_Product_Code=01

# Server Port
PORT=8001  # Due to port 8000 conflicts
```

> [!IMPORTANT]
> **실전 투자(Production)**와 **모의 투자(Sandbox)** 모드는 `KIS_ENV` 변수로 제어됩니다.
> - `KIS_ENV=production`: 실전 계좌 연결 (주의!)
> - `KIS_ENV=sandbox`: 모의 투자 계좌 연결
>
> 소스 코드 내에서 `is_virtual` 파라미터를 명시하지 않으면 이 환경변수 설정을 따릅니다.
